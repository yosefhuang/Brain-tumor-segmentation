...
from dataset_change import BrainSegmentationDataset as Dataset
from unet import UNet,NestedUNet
from utils import dsc, gray2rgb, outline, my_dsc, dsc_temp
...
def postprocess_per_volume(
    ...
    for p in range(len(num_slices)):
        ...
        volume_pred = np.array(pred_list[index : index + num_slices[p]])
        volume_pred[volume_pred < 0.7] = 0.0
        volume_pred = np.round(volume_pred).astype(int)
...
def dsc_distribution(volumes):
    ...
    for p in volumes:
        y_pred = volumes[p][1]
        y_true = volumes[p][2]
        dsc_dict[p] = dsc_temp(y_pred, y_true, lcc=False)
    return dsc_dict
